<template>
    ...
    <transition name="overlay">
        ...
        <div class="overlay" v-show="overlayPopupStates[activeOverlayPopup]" @click="popupMethodStore().closeOverlayPopup()">
            ...
            <!-- 蒙版弹窗组件动画 -->
            <transition :name="overlayPopupStates[activeOverlayPopup]?.animation">
...
/*
 * Vue 的 <transition name="overlay"> 在进入与离开阶段，会自动给元素添加或移除如下类名：
 * 1. .overlay-enter-active：进入过渡激活阶段，表示进入动画即将开始
 * 2. .overlay-enter-from：进入开始阶段，表示进入动画的第一帧
 * 3. .overlay-enter-to：进入结束阶段，表示进入动画的最后一帧
 * 4. .overlay-leave-active：离开过渡激活阶段，表示离开动画即将开始
 * 5. .overlay-leave-from：离开开始阶段，表示离开动画的第一帧
 * 6. .overlay-leave-to：离开结束阶段，表示离开动画的最后一帧
 *
...
// 当弹窗组件执行 进入动画的第一帧 或 离开动画的最后一帧 时
...
// 当弹窗组件执行 进入动画的最后一帧 或 离开动画的第一帧 时
